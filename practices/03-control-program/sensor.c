...
#pragma pack(1)
#if defined(NETWORK_EMULATE) || defined(FILE_EMULATE)
/* Регистры заполняются только эмулятором: без него массив не нужен. */
...
static void update_file_registers(struct EmulateRegisters *em) {
    if (em->fd != 0) {
        ...
        fscanf(em->fd, "%d %d %d %d %d %d %d %d %d %d %d %d %d", &registers[SENSOR_POWER_OFF],
...
static void simulate_update(void) {
    if (emulator.started == false) {
#if defined(NETWORK_EMULATE)
        memset(&emulator, 0, sizeof(emulator));
#if defined(_WIN32)
        emulator.thread_h = CreateThread(NULL, 0, network_server_thread, (void *)&emulator, 0,
                                         &emulator.thread_id);
        if (emulator.thread_h != INVALID_HANDLE_VALUE) {
            CloseHandle(emulator.thread_h);
        }
#else
        if (pthread_create(&emulator.server_thread, 0, network_server_thread, (void *)&emulator)
            == 0) {
...
static const char *get_name(enum Sensor id) {
    static char buf[20];
    switch (id) {
    case SENSOR_D1:
    case SENSOR_D2:
...
void emulator_sensor_init(struct SensorInterface *si) {
