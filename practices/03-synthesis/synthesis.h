...
#define SYN_MAX_SYMBOLS 16
#define SYN_MAX_STATES 16
#define SYN_MAX_NAME 32
#define SYN_MAX_BITS BOOL_MAX_VARS
...
struct Machine {
    ...
    int state_count;
    /* next[state][input] и emit[state][input] — индексы состояния и выхода;
       -1 означает, что переход не задан (безразличный набор). */
...
/* Число разрядов, нужное для кодирования count символов. */
...
/* Порождение реализации автомата на C по полученным формулам. */
