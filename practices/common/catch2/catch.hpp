...
namespace Catch {
    ...
    // ЛОКАЛЬНАЯ ПРАВКА (не из upstream Catch2 v2.3.0): в оригинале здесь
    // безусловная x86-вставка "int $3", из-за которой заголовок не
    // компилируется на Apple Silicon (arm64). Ветка по архитектуре взята из
    // более поздних версий Catch2.
    #if defined(__i386__) || defined(__x86_64__)
        #define CATCH_TRAP() __asm__("int $3\n" : : ) /* NOLINT */
    #elif defined(__aarch64__) || defined(__arm64__)
        #define CATCH_TRAP() __asm__(".inst 0xd4200000") /* NOLINT */
    #endif
    /* На прочих архитектурах CATCH_TRAP не определяется — Catch2 сам
       подставит Catch::doNothing(). */
