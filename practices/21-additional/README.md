...
Последнее задание — тот отказ, при котором датчики положения молчат: линия под
погрузчиком, меток нет, препятствий нет, а машина стоит. Ловит его сторожевой
таймер команды, `sense_motion` подсвечивается красным, и погрузчик уходит в
аварию.

```bash
./21-additional-run --mission mission/pick-and-place.json     # в консоли
./21-additional-gui --mission mission/jam.json --speed 240    # в окне
```
