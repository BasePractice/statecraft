...
**Режим сценария.** Ключ `--scenario` или перетаскивание файла в окно
переключают прогон на сценарий — тот же `scenario/loader.json`, которым модель
проверяется в тестах. Тогда значения портов берутся из файла по шагу за такт, а
установка не тикает: так разбираются отказы, которых от исправной установки не
дождёшься — чужая метка, пропавшая линия, препятствие вплотную.
