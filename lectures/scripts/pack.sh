...
# Сборка с --pretty оставляет только файлы под читаемыми именами, поэтому
# отбор по разделителю в имени — страховка: если в каталог попадёт что-то со
# сборочным именем, в комплект оно не поедет.
count=0
