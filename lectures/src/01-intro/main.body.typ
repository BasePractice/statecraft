...
#definition(name: "алфавит")[
  #emph[Алфавитом] называется конечное непустое множество $A$, элементы
  которого называются #emph[буквами] (символами).
]

#definition(name: "слово")[
  #emph[Словом] (цепочкой) в алфавите $A$ называется конечная
...
Обозначим через $A^(*)$ множество всех слов в алфавите $A$, включая пустое, а
через $A^(+) = A^(*) without { epsilon.alt }$ --- множество всех непустых
слов @Hopcroft[с. 40--43]. Через $A^(oo)$ обозначается множество #emph[сверхслов] ---
...
#definition(name: "язык")[
  ...
  алфавите $A$ называется произвольное подмножество $L subset.eq A^(*)$.
]

#example[
  Пусть $A = { a, b }$. Тогда:
  - $L_1 = { a, a b, a b b }$ --- конечный язык из трёх слов;
  - $L_2 = { alpha in A^(*) : |alpha|_a "чётно" }$ --- язык слов с чётным
