...
- `template/diagrams.typ` — единственный файл с `@preview`-зависимостью
  (fletcher). Подключается только теми лекциями, где есть векторные
  диаграммы; кэш прогревается `scripts/vendor-packages.sh`.
...
Всё помечено в PDF красными блоками «НЕ НАПИСАНО» (`#todo[...]`), поиск:
...
- карты Карно `KA_Table_3..12_MoneyAutomate.png` в лекции 3 остаются
  растровыми снимками книжной страницы и плохо читаются; их значения
  однозначно выводятся из перекодированной таблицы, набранной нативно.
  Таблицы переходов и диаграмма Мура разменного аппарата уже переведены
  в векторный вид;
